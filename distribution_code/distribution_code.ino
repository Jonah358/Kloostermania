...
class Game {
  public:
    ...
    void setupGame() {
      ...
      if (level == 1) {
        for (int i = 0; i < 8; i++) {
          enemies[i].initialize((i * 4), 0, 3);
          enemies[i].draw();
        }
        for (int j = 8; j < 16; j++) {
          enemies[j].initialize((count * 4), 4, 2);
          enemies[j].draw();
          count++;
        }
      }
    ...
    void update(int potentiometer_value, bool button_pressed) {
      ...
      if(ball.has_been_fired() && (millis() - ballTime > 33)){
        ballTime = millis();
        ball.move();
      }
     ...
     else if (dead == 8 && moveInvader() == true) {
      ...
      // FIXME: read color of invader pixel, if black, do nothing, otherwise hit()
      // FIXME: Invaders losing pixels, bottom pixels need to be protected
